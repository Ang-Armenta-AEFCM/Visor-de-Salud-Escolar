...
function updateActiveFilters() {
  const filters = [
    ['Alcaldía', $('filterAlcaldia').value], ['Ciclo', $('filterCycle').value],
...
function showDetails(record, focusMap = true) {
  ...
  const baseInfo = [
    ...
    optionalInfo('Nivel educativo', record.NIVEL_EDUCATIVO),
  ...
  $('detailsBody').innerHTML = `
    <section class="detail-section"><div class="school-detail-header"><div class="school-detail-icon"><i class="fa-solid fa-school"></i></div><div><h3>${escapeHtml(record.NOM_ESCUELA_ATENCION)}</h3><p>${escapeHtml(record.CVE_ESCUELA_ATENCION)} · ${escapeHtml(record.NIVEL_EDUCATIVO || '')}</p></div></div></section>
...
function renderSearch(query) {
  ...
  $('searchResults').innerHTML = matches.length ? matches.map(row =>
    `<button class="search-result-item" type="button" data-id="${row._id}"><strong>${escapeHtml(row.NOM_ESCUELA_ATENCION)}</strong><span>${escapeHtml(row.CVE_ESCUELA_ATENCION)} · ${escapeHtml(row.NIVEL_EDUCATIVO)}</span><small>${escapeHtml(row.alcaldia)}</small></button>`
...
function selectSearchResult(id) {
  ...
  ['filterAlcaldia', 'filterCycle', 'filterPriority', 'filterAlerts'].forEach(filterId => $(filterId).value = '');
...
function clearFilters() {
  ['filterAlcaldia', 'filterCycle', 'filterPriority', 'filterAlerts'].forEach(id => $(id).value = '');
  $('searchInput').value = '';
  closeDetails();
  applyFilters();
...
['filterCycle', 'filterPriority', 'filterAlerts'].forEach(id => $(id).addEventListener('change', applyFilters));
